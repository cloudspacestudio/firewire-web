...
const sqldb_1 = require("./workspaces/fieldwire/repository/sqldb");
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const app = yield new bootstrap_1.Bootstrap().start(true);
        const sqldb = new sqldb_1.SqlDb(app);
        yield sqldb.ensurePartsSchema();
        ...
        for (const sheetName of workbook.SheetNames) {
            ...
            const vendor = yield upsertWorkbookVendor(sqldb, sheetName);
            ...
            yield sqldb.replacePartsForVendor(vendor.vendorId, normalizedRows);
            yield sqldb.createVendorImportRun({
                vendorId: vendor.vendorId,
                targetTable: 'parts',
                fileName: path.basename(workbookPath),
                snapshotId,
...
function upsertWorkbookVendor(sqldb, sheetName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const existing = yield sqldb.getVendorByName(name);
        if (existing) {
            yield sqldb.updateVendor(Object.assign(Object.assign({}, existing), { desc: existing.desc || 'Imported security pricing vendor.', link: existing.link || '', importConfigJson }));
            const reloaded = yield sqldb.getVendorById(existing.vendorId);
            if (!reloaded) {
                throw new Error(`Vendor ${existing.vendorId} could not be reloaded after update.`);
            }
            return reloaded;
        }
        const vendorId = yield sqldb.createVendor({
            name,
            desc: 'Imported security pricing vendor.',
            link: '',
            importConfigJson
        });
        const created = yield sqldb.getVendorById(vendorId);
        if (!created) {
...
function normalizeWorkbookRow(sheetName, row) {
    ...
    return {
        ...
        countryOfOrigin: '',
        upc: '',
        rawJson: JSON.stringify(Object.assign(Object.assign({}, row), { serviceDescription: limitText(serviceDescription, 2000), manufacturerOrReseller: limitText(manufacturerOrReseller, 500), discountPercent: readPercent(row, 'DISCOUNT % OFF MSRP'), dirAdminFee: readMoney(row, 'DIR ADMIN FEE') }))
    };
}
function readText(row, key) {
    var _a;
    return String((_a = row[key]) !== null && _a !== void 0 ? _a : '').trim();
}
function readPartNumber(row, key) {
