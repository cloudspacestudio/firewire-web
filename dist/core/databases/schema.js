...
exports.DataType = exports.Schema = void 0;
class Schema {
    constructor() {
        this.name = '';
        ...
        this.format = undefined; // format: email, phone, uri, hostname, ipv4, ipv6, uuid, cidr, date, datetime
    ...
    static getDataTypeFromString(input) {
        if (!input)
    ...
    static splitStringPreservingQuotes(input) {
        const result = [];
        let currentSegment = '';
        let inQuotes = false;
    ...
    static parseEntityJson(entityName, text) {
        ...
        try {
            ...
            Object.keys(jsonified).forEach((key) => {
    ...
    static parseJsonDocument(entityName, input) {
        const output = [];
        if (!input)
            return output;
        try {
            ...
            if (!countFields || countFields.length <= 0) {
                input = input.replace(/([a-zA-Z0-9_]+)\s*: /g, (match, key) => `"${key}":`);
            }
    ...
    static parseJSONSchema(entityName, input) {
        const output = [];
        if (!input || !entityName)
            return output;
        try {
    ...
    static toJsonSchema(entityName, items) {
    ...
    static toTypeScriptInterface(entityName, items) {
        if (!items || !Array.isArray(items) || items.length <= 0)
            return undefined;
    ...
    static toTypeScriptClass(entityName, items) {
        if (!items || !Array.isArray(items) || items.length <= 0)
            return undefined;
        const itemStrings = items.map((item) => {
    ...
    static toSqlServerDDL(entityName, items) {
        if (!items || !Array.isArray(items) || items.length <= 0)
            return undefined;
        const itemStrings = items.map((item) => {
            const itemDataType = Schema._toTypeScriptDataType(item.type);
    ...
    static getFieldValueForKey(item, key) {
        if (!item || !key)
            return undefined;
        //itemAny[key]
        switch (key) {
    ...
    static _toTypeScriptDataType(fieldType) {
        switch (fieldType) {
            case DataType.boolean:
                return 'bool';
            case DataType.date:
    ...
    static _getTypeScriptDataTypeDefault(fieldType) {
        switch (fieldType) {
            case DataType.boolean:
                return false;
            case DataType.date:
    ...
    static _toSqlServerColumnString(item) {
    ...
    static _fromSqlServerDataType(input) {
        if (!input)
            return DataType.string;
    ...
    static _replaceAllInstances(target, search, replacement) {
